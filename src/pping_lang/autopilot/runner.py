...
每个 session 结束必写一条带 stop_cause 的 stop 轮(含判停时桌面快照),停机原因可统计。
"""
...
class Runner(threading.Thread):
    ...
    def _run_baseline(self) -> None:
        ...
        except Exception as e:           # LaunchError/BenchError → 先尝试 recovery 自愈
    ...
    def _run_baseline_recovery(self, error: str) -> Scorecard | None:
        ...
        for attempt in range(1, MAX_RECOVERY + 1):
            failure_ctx = {
                "error": error,
                "error_type": self._classify_failure(error),
                "container_log_tail": self._sb._logs_tail() if hasattr(self._sb, "_logs_tail") else "",
                "current_baseline_config": dict(self._baseline),
                "bench_plan": self._bench_plan(),
                "tried_recovery_actions": list(tried),
                "attempt": attempt,
    ...
    def _classify_failure(self, error: str) -> str:
        err = error.lower()
        if "outofmemory" in err or "oom" in err or "cuda out of memory" in err:
            return "OOM"
    ...
    def _apply_recovery_action(self, action: str) -> bool:
        """执行 recovery 动作,返回是否发生了实际变更。"""
        changed = False
        if action == "raise_gpu_memory_utilization":
            old = float(self._baseline.get("gpu_memory_utilization", 0.9))
            new = round(min(old + 0.05, 0.98), 2)
            if new > old:
                self._baseline["gpu_memory_utilization"] = new
                changed = True
        elif action == "lower_bench_concurrency":
            spec = getattr(self._sb, "_spec", None)
            if spec:
                old = int(spec.get("concurrency", 32))
                new = max(old - 8, 16)
                if new < old:
                    spec["concurrency"] = new
                    changed = True
        elif action == "lower_baseline_max_num_seqs":
            old = int(self._baseline.get("max_num_seqs", 64))
            new = max(int(old * 0.75), 16)
            if new < old:
                self._baseline["max_num_seqs"] = new
                changed = True
        elif action == "lower_max_model_len":
            old = int(self._baseline.get("max_model_len", 2048))
            new = 1024
            if new < old:
                self._baseline["max_model_len"] = new
                changed = True
        return changed
    ...
    def _run_loop(self, *, start_round: int = 1, history: list[dict] | None = None,
        ...
        rnd = start_round
        diag: dict | None = None                       # 供自然耗尽路径的 stop 轮带上最后一次诊断
        forced_rel_streak = 0                          # 相对门槛(③)的连续强制计数
        while (not self._stopping.is_set() and rnd <= self._rounds_budget
            ...
            if dec.done:
                ...
                untried_n = sum(1 for c in cands
                                if config_hash(c["config"]) not in tried_hashes)
                g_explore = prior_explored < MIN_EXPLORE_ROUNDS
                g_relative = untried_n > len(cands) / 2 and forced_rel_streak < MAX_FORCED_RELATIVE
                if (g_explore or sla_never_met or g_relative) and rounds_left > 1 and cands:
                    failed = {t["hash"] for t in tried
                ...
                forced_rel_streak = 0               # agent 正常提议,强制连击清零
    ...
    def _borderline(self, sc: Scorecard, before_score: float, score: float) -> str | None:
        """成绩贴近判定边界(Δ 在噪声带内 / p99 距 SLA 边界 <10%)→ 补测理由;否则 None。"""
        reasons: list[str] = []
                ...
                if not ok:
                    ...
                    if deadline is not None and time.monotonic() >= deadline:
                        ...
                        if why and self._bench_repeats == 1:
                            ...
                            sc.run_meta["search_mode"] = self._search_mode
                            sc.run_meta["repeats_trigger"] = "borderline"
                            score = objective_score(sc, self._obj)
    ...
    def _table_snapshot(self, cands: list[dict] | None, tried: list[dict], p0) -> list[dict]:
        ...
        snap = [{"knob": c.get("knob"), "from": c.get("from"), "to": c.get("to"),
                 "tried": by_hash.get(config_hash(c["config"]), "untried"),
                 "p0": "kept"}
                for c in cands or []]
        for pc in getattr(p0, "pruned", None) or []:
            snap.append({"knob": pc.get("knob"), "from": pc.get("from"), "to": pc.get("to"),
                         "tried": "untried",
                         "p0": "pruned:" + str((pc.get("p0") or {}).get("reason", ""))[:80]})
        return snap

    def _append_stop(self, rnd: int, dec, diag: dict | None, *,
                     cause: str, table: list[dict] | None = None) -> None:
        self._store.append_round(Round(
            round=rnd, kind="stop", state_at_record="deciding", decision="done",
            rationale=dec.rationale or dec.reason, evidence_refs=dec.evidence_refs,
            diagnosis=diag, stop_cause=cause, table_snapshot=list(table or []),
            agent_model=getattr(self._agent, "model", ""),
            agent_thinking=(getattr(dec, "thinking", "") or "")[:4000]))
        self._tick()
