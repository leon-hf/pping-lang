...
# 各业务形态的 SLA 默认 (TTFT_p99_ms, TPOT_p99_ms, E2E_p99_ms) —— 见 §12 黄金阈值表 + 业界惯例
# (交互聊天 TTFT 数百 ms、内联代码补全个位数百 ms、RAG/Agent 因检索或工具往返放宽、长推理
# 输出以千 token 计因此 E2E 放到分钟级)。选了 workload_form 就带出这套默认;仍可被显式字段覆盖。
_WORKLOAD_SLA: dict[str, tuple[float, float, float]] = {
...
def default_config(workload_form: str = "custom") -> DiagnosisConfig:
    ...
    ttft, tpot, e2e = _WORKLOAD_SLA[form]
    return DiagnosisConfig(
        workload_form=form, sla_ttft_p99_ms=ttft, sla_tpot_p99_ms=tpot, sla_e2e_p99_ms=e2e,
    )
...
def validate_config(cfg: DiagnosisConfig) -> None:
    ...
    for f in ("sla_ttft_p99_ms", "sla_tpot_p99_ms", "sla_e2e_p99_ms", "long_prompt_tokens",
    ...
    if cfg.mfu_low_ratio >= cfg.mfu_high_ratio:
        raise ValueError("mfu_low_ratio must be < mfu_high_ratio(否则'喂不饱'与'算力瓶颈'区间重叠)")
...
def load_config() -> DiagnosisConfig:
    ...
    try:
        ...
        cfg = from_dict(raw)
        logger.info(
            "[pping-lang] loaded DiagnosisConfig: workload=%s ttft_sla=%.0fms tpot_sla=%.0fms e2e_sla=%.0fms",
            cfg.workload_form, cfg.sla_ttft_p99_ms, cfg.sla_tpot_p99_ms, cfg.sla_e2e_p99_ms,
        )
        return cfg
